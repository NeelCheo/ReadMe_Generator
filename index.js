...
function writeToFile(data, category) {
  if (questions[category] == "title") { // title is handled diffrently since it just the section header no content
    ...
    //   if (questions[header] !== "title","username","email"){
    ...
    //     });
    // }}
  } else if (questions[category] == "username") { // user name requires more than just the user input so it has it own section
    fs.appendFileSync("README.md", "## questions?" + "\n", (err) => {
      if (err) {
        console.error(err);
      }
  ...
  } else if (questions[category] == "email") { // email requires more than just the user input so it has it own section
    fs.appendFileSync("README.md", "\n" + "email: " + data[questions[category]] + "\n", (err) => {
      if (err) {
        console.error(err);
      }
    });
  } else { // this handles most of the sections
